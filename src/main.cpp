...
#if defined(NDEBUG)
# error "Letecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdfdbaaaac7cef5306f85b897a14075693796733a6b13e71f19fb17f02871ee5a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Letecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Letecoin Signed Message:\n";
...
{
    // Letecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 113,000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 113000); // Letecoin: 113k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // Letecoin: 4 days
static const int64 nTargetSpacing = 2 * 60; // Letecoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Letecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Letecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[3] = 0xdl;
        hashGenesisBlock = uint256("0x5f0ad4b4eb3e89ed50a218df6effebaa2d395abb7039dd35e7b6496563484e30");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Jun/2017 Something very important happened.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04374e4748d19dcca4d4e3dce13f23689179ef100c70522ccca810804c2920bc91d30ec64918827ad2bf5b3e7dea793a0be86e82dfb48c55df9b22256ed74f5953") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515789161;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084994296;

        if (fTestNet)
        {
            block.nTime    = 1515789129;
            block.nNonce   = 386458431;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfcf2e458907dbbfeb065362caf2be2b0be02d02387138ee96d5b5d8acfa96c29"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb3, 0xda }; // Letecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LetecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LetecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LetecoinMiner(CWallet *pwallet)
{
    printf("LetecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("letecoin-miner");
    ...
    try { loop {
        ...
        printf("Running LetecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LetecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LetecoinMiner, pwallet));
}
